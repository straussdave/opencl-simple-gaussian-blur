...
namespace Gaussian_Blur
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IMem<byte> imageBuffer = Cl.CreateBuffer<byte>(
                context,
                MemFlags.ReadOnly,
                imageDataSize,
                out status);
            CheckStatus(status);
            IMem<float> gausKernelBuffer = Cl.CreateBuffer<float>(
                context,
                MemFlags.ReadOnly,
                gausKernel.Length * sizeof(float),
                out status);
            CheckStatus(status);
            IMem<byte> tempBuffer = Cl.CreateBuffer<byte>(
                context,
                MemFlags.ReadWrite,
                imageDataSize,
                out status);
            CheckStatus(status);
            IMem<byte> outputBuffer = Cl.CreateBuffer<byte>(
                context,
                MemFlags.WriteOnly,
                imageDataSize,
                out status);
            ...
            CheckStatus(Cl.EnqueueNDRangeKernel(commandQueue, kernel, 2, null, new IntPtr[] { (IntPtr)width, (IntPtr)height }, localSizeRow, 0, null, out _));

            //// WHY DOES IT WORK WITHOUT ENQUEUEING WRITE BUFFER
            ///
            ...
            CheckStatus(Cl.EnqueueNDRangeKernel(commandQueue, kernel, 2, null, new IntPtr[] { (IntPtr)width, (IntPtr)height }, localSizeCol, 0, null, out var _));
            ...
            Cl.ReleaseMemObject(gausKernelBuffer);
            Cl.ReleaseMemObject(tempBuffer);
